Thankyou for visiting my space.. <br>
This repository focuses on the skills that junior SOC analyst needs. <br> 
From digging through logs and triaging alerts to working with teammates and responding to security incidents. <br>
The goal is to build strong technical and operational foundations for real-world SOC analyst. <br>

## 📊 Progress Status 
### Splunk Module
| No. | Module Name                         | Focus Area                                      | Status        |
|-----|------------------------------------|------------------------------------------------|--------------|
| 1   | Splunk: Exploring SPL              | Search Processing Language (SPL) fundamentals   | ⬜ Not Started |
| 2   | Splunk: Setting up a SOC Lab       | Lab setup & log ingestion (Universal Forwarder)| ⬜ Not Started |
| 3   | Splunk: Dashboards and Reports     | Alerts, reports, and visualization             | ⬜ Not Started |
| 4   | Splunk: Data Manipulation          | Parsing & transforming log data                | ⬜ Not Started |
| 5   | Fixit Challenge                    | Log parsing issue & investigation              | ⬜ Not Started |

